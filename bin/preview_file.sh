...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        7z)
            ...
            ## PDF
        pdf)
            # if [[ ! -f "/tmp/$FILE_BASE_NAME.png" ]]; then
            #     mutool draw -r 150 -o "/tmp/$FILE_BASE_NAME.png" "${FILE_PATH}"
            # fi

            # kitty +kitten icat --silent --stdin no --transfer-mode file --place "${PREVIEW_WIDTH}x${PREVIEW_HEIGHT}@${PREVIEW_X}x${PREVIEW_Y}" "/tmp/$FILE_BASE_NAME.png" 2>/dev/null
            exiftool "${FILE_PATH}" && exit 0

            # handle_mime "image/png" "preview" && exit 0
